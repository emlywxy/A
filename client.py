...
def receive_message(connection: socket.socket) -> Optional[dict[str, Any]]:
    global _recv_buf
    while True:
        nl = _recv_buf.find(b"\n")
        if nl != -1:
...
def disconnect(connection: Optional[socket.socket]) -> None:
    if not connection:
        return
    try:
...
def answer_question(
    ...
    if client_mode == "auto":
        ...
        if "Roman Numerals" in question:
            ...
            total = 0
            index = 0
            while index < len(short_question):
                value = table[short_question[index]]
                if index + 1 < len(short_question) and table[short_question[index + 1]] > value:
                    total += table[short_question[index + 1]] - value
                    index += 2
                else:
                    total += value
                    index += 1
        ...
        if "Network and Broadcast Address of a Subnet" in question:
            ...
            broadcast = f"{(bcast >> 24) & 255}.{(bcast >> 16) & 255}.{(bcast >> 8) & 255}.{bcast & 255}"
            return f"{network} and {broadcast}"
...
def load_client_config(argv: list[str]) -> dict[str, Any]:
    ...
    if config.get("client_mode") == "ai":
        ...
        required = ("ollama_host", "ollama_port", "ollama_model")
        if not ollama_cfg or any(key not in ollama_cfg for key in required):
...
def receiver_loop(connection: socket.socket, mode: str, cfg: dict[str, Any]) -> None:
    def arm_timeout(seconds: float) -> None:
    ...
    while True:
        try:
            ...
            if message.get("message_type") == "QUESTION":
                time_limit = float(message.get("time_limit"))
                arm_timeout(time_limit)
...
def main() -> None:
    ...
    def read_line_with_small_timeout() -> Optional[str]:
        try:
        ...
        while True:
            if question_ready.is_set() and mode in ("auto", "ai") and sock is not None:
                ...
                if question is not None:
                    try:
                        time_limit = float(question.get("time_limit"))
                        answer = run_with_timeout(
                            time_limit,
                            answer_question,
                            question.get("trivia_question"),
                            question.get("short_question"),
                            mode,
                            time_limit,
                            cfg.get("ollama_config") if mode == "ai" else None,
                        )
                        if waiting_for_answer.is_set() and answer:
                            try:
                                send_message(sock, {"message_type": "ANSWER", "answer": answer})
                            except OSError:
                                pass
                    except TimeoutExpired:
                        pass
            ...
            if mode == "you" and waiting_for_answer.is_set() and sock is not None:
                try:
